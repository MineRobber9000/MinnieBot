...
static IntrinsicResult intrinsic_File(Context *context, IntrinsicResult partialResult) {
	return IntrinsicResult::Null;
#if 0 // TODO: find a way to properly sandbox file access
...
static void setEnvVar(const char* key, const char* value) {
	...
	envMap.SetValue(keyStr, valueStr);
#if 0
	#if WINDOWS
		_putenv_s(key, value);
	#else
...
static IntrinsicResult intrinsic_import(Context *context, IntrinsicResult partialResult) {
	...
	Value searchPath("$MS_EXE_DIR/lib"); // = envMap.Lookup(_MS_IMPORT_PATH, Value::null);
	StringList libDirs;
...
void AddScriptPathVar(const char* scriptPartialPath) {
	String scriptDir("/home/nunya/business");
#if 0
...
void AddPathEnvVars() {
	envMap.SetValue(_MS_IMPORT_PATH, "$MS_EXE_DIR/lib");
